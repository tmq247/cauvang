...
async def fdel(client, message):
    ...
    user1 = await ubot.get_users("tg://openmessage?user_id=" + user_id)
    for served_chat in served_chats:
        try:
            try:
                chat_member = await app.get_chat_member(served_chat, user.id)
                chat_member1 = await ubot.get_chat_member(served_chat, user1.id)
            ...
            print(y_e)
            await app.send_message(chat.id, "Lỗi")
            pass
